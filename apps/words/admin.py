...
class EnglishWordAdmin(OwnershipAdminMixin):
    ...
    search_fields = ('readings__romaji', 'readings__hiragana',
                     'readings__katakana', 'word', 'tags__eng_tag',
                     'tags__jap_tag')
...
class JapaneseWordAdmin(OwnershipAdminMixin):
    ...
    def add_list(self, request):
        ...
        JpWordFormset = formset_factory(wraps(forms.JpWordForm)(partial(
            forms.JpWordForm, user=request.user)),
        ...
        return render(request, "admin/words/japaneseword/add_list.html",
                      context)
